...
import { searchContent, TMDBResult, getTMDBImageUrl, getTMDBBackdropUrl, getContentDetails } from './utils/tmdb';
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 flex flex-col selection:bg-[hsl(var(--primary)/0.3)] relative overflow-x-hidden">
              ...
              {selectedContent ? (
                <motion.div key="selected" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-6">
                  <div className="flex flex-col sm:flex-row gap-6 items-start sm:items-center">
                    <div className="w-32 h-48 rounded-2xl overflow-hidden shadow-2xl border-2 flex-shrink-0 bg-zinc-900 ring-4 ring-black/50" style={{ borderColor: 'hsl(var(--primary) / 0.3)' }}>
                      <img src={getTMDBImageUrl(selectedContent.poster_path)} alt={selectedContent.title || selectedContent.name} className="w-full h-full object-cover" />
                    </div>
                    <div className="space-y-3">
                      <h1 className="text-4xl lg:text-5xl font-extrabold tracking-tight leading-tight font-display">{selectedContent.title || selectedContent.name}</h1>
                      <div className="flex flex-wrap items-center gap-3 text-zinc-400">
                        <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-white/5 text-sm font-medium"><Calendar size={14} className="text-zinc-500" /> {(selectedContent.release_date || selectedContent.first_air_date || '').substring(0, 4)}</span>
                        <span className="px-3 py-1 rounded-lg text-sm font-bold uppercase tracking-wider" style={{ backgroundColor: 'hsl(var(--primary) / 0.2)', color: 'hsl(var(--primary))' }}>{selectedContent.media_type === 'movie' ? 'Filme' : 'Série'}</span>
                      </div>
                    </div>
                  </div>
                  <p className="text-zinc-400 text-lg leading-relaxed line-clamp-4 max-w-xl">{selectedContent.overview || 'Nenhuma descrição disponível.'}</p>
                  <button onClick={() => { setSelectedContent(null); navigate('/', { replace: true }); }} className="group inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-sm font-bold hover:bg-white/10 transition-all active:scale-95"><X size={16} className="text-zinc-500 group-hover:text-red-400" /> Limpar Escolha</button>
                </motion.div>
              ) : (
                <motion.div key="hero" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-6">
                  <h1 className="text-5xl lg:text-7xl font-extrabold tracking-tighter leading-[1.1] font-display">O Cinema na <br/><span className="text-transparent bg-clip-text transition-all duration-1000" style={{ backgroundImage: 'linear-gradient(to right, hsl(var(--primary)), hsl(var(--primary-accent)), hsl(var(--primary)))', backgroundSize: '200% auto' }}>Sua Mão</span></h1>
                  <p className="text-zinc-400 text-xl max-w-md leading-relaxed font-medium">Integre sua biblioteca do TMDB com o player customizado mais moderno do mercado.</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
            ...
            <form onSubmit={handlePlay} className="space-y-8">
              <div ref={searchRef} className="space-y-3 relative">
                ...
                <div className="relative group">
                  <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSearch(e)} placeholder="Filme ou Série..." className="w-full bg-black/40 border-2 border-white/5 rounded-2xl px-5 py-4 pr-12 text-zinc-100 placeholder:text-zinc-600 focus:outline-none focus:border-[hsl(var(--primary)/0.5)] transition-all shadow-inner" />
                  <button type="button" onClick={() => handleSearch()} disabled={isSearching} className="absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-xl flex items-center justify-center text-zinc-500 hover:text-white transition-all active:scale-90">{isSearching ? <Loader2 size={20} className="animate-spin" /> : <ChevronRight size={24} />}</button>
                </div>
                <AnimatePresence>
                  {searchResults.length > 0 && (
                    <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className="absolute top-full left-0 right-0 mt-3 bg-zinc-900/95 border border-white/10 rounded-2xl shadow-2xl z-[100] max-h-[400px] overflow-y-auto scrollbar-hide backdrop-blur-xl divide-y divide-white/5 ring-1 ring-black">
                      {searchResults.map((result) => (
                        <button key={`${result.media_type}-${result.id}`} type="button" onClick={() => selectResult(result)} className="w-full flex items-center gap-4 p-4 hover:bg-white/[0.03] text-left transition-all group">
                          <div className="w-14 h-20 rounded-xl overflow-hidden bg-zinc-800 flex-shrink-0 shadow-lg group-hover:scale-105 transition-transform">
                            <img src={getTMDBImageUrl(result.poster_path)} className="w-full h-full object-cover" alt={result.title || result.name} />
                          </div>
                          <div className="flex-1 min-w-0 space-y-1">
                            <div className="text-base font-bold text-zinc-100 truncate group-hover:text-[hsl(var(--primary))] transition-colors">{result.title || result.name}</div>
                            <div className="flex items-center gap-3 text-sm text-zinc-500"><span className="font-mono">{(result.release_date || result.first_air_date || '').substring(0, 4)}</span> <span className="px-2 py-0.5 rounded-md text-[10px] uppercase font-bold transition-colors" style={{ backgroundColor: 'hsl(var(--primary-muted) / 0.2)', color: 'hsl(var(--primary))' }}>{result.media_type === 'movie' ? 'Filme' : 'Série'}</span></div>
                          </div>
              ...
              <div className="space-y-5">
                <div className={selectedContent ? 'opacity-40' : ''}>
                  <label className="text-sm font-bold text-zinc-400 block mb-2 px-1 uppercase">ID Manual</label>
                  <input type="text" required value={contentId} onChange={(e) => setContentId(e.target.value)} placeholder="TMDb ID" className="w-full bg-black/30 border border-white/10 rounded-2xl px-5 py-4 text-zinc-100 focus:outline-none focus:border-[hsl(var(--primary)/0.3)] transition-all" />
                </div>
                <AnimatePresence>
                  {(contentType === 'tv' || contentType === 'anime-show') && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="grid grid-cols-2 gap-4 overflow-hidden">
                      {contentType === 'tv' && (
                        <div className="space-y-2"><label className="text-[10px] font-bold tracking-widest text-zinc-500 uppercase px-1">Temporada</label><input type="number" min="1" value={seasonNum} onChange={(e) => setSeasonNum(e.target.value)} placeholder="1" className="w-full bg-black/30 border border-white/10 rounded-2xl px-5 py-3.5 text-zinc-100 focus:outline-none focus:border-[hsl(var(--primary)/0.3)] transition-all" /></div>
                      )}
                      <div className="space-y-2"><label className="text-[10px] font-bold tracking-widest text-zinc-500 uppercase px-1">Episódio</label><input type="number" min="1" value={episodeNum} onChange={(e) => setEpisodeNum(e.target.value)} placeholder="1" className="w-full bg-black/30 border border-white/10 rounded-2xl px-5 py-3.5 text-zinc-100 focus:outline-none focus:border-[hsl(var(--primary)/0.3)] transition-all" /></div>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
              ...
              <div className="space-y-6 pt-6 border-t border-white/5">
                <div className="flex items-center justify-between">
                  <div className="space-y-1"><h3 className="text-sm font-bold text-zinc-200">Personalização</h3><p className="text-[10px] text-zinc-500 font-medium uppercase tracking-tighter">A cor afeta toda a interface do app</p></div>
                  <div className="flex items-center gap-3 bg-black/20 p-2 rounded-2xl border border-white/5">
                    <div className="relative w-10 h-10 rounded-xl overflow-hidden cursor-pointer active:scale-95 transition-transform shadow-lg border border-white/10" style={{ backgroundColor: themeColor.startsWith('#') ? themeColor : `#${themeColor}` }}><input type="color" value={themeColor.startsWith('#') ? themeColor : `#${themeColor}`} onChange={(e) => setThemeColor(e.target.value.replace('#', ''))} className="absolute inset-0 opacity-0 cursor-pointer w-full h-full scale-150" /></div>
              ...
              <motion.button whileHover={{ scale: 1.02, brightness: 1.1 }} whileTap={{ scale: 0.98 }} type="submit" className="w-full py-5 px-8 rounded-2xl font-bold text-xl tracking-tight transition-all shadow-2xl flex items-center justify-center gap-3 relative overflow-hidden group font-display" style={{ backgroundColor: 'hsl(var(--primary))', color: 'white' }}>
      ...
      <footer className="p-8 border-t border-white/5 flex flex-col sm:flex-row justify-between items-center gap-4 text-zinc-600 text-[10px] font-bold uppercase tracking-widest relative z-10">
        <div className="flex items-center gap-6"><span>Videasy v2.4</span> <span>TMDB API Ready</span> <span className="text-[hsl(var(--primary))]">PWA Optimized</span></div>
        <div className="flex items-center gap-2"><span>Design by IronCripto</span><div className="w-1 h-1 rounded-full bg-zinc-800" /><span>© 2026</span></div>
      </footer>
    </div>
  );
}
